...
public class WormLavaESP extends Module {
   ...
   public WormLavaESP() {
      super("WormLavaESP", "Auto Find Lava that can be used to worm fish", -1, Module.Category.RENDER);
   ...
   public void onTick(TickEvent event) {
      if (mc.world != null && mc.player != null) {
         if ((double)(System.currentTimeMillis() - this.lastSearch) >= 1000.0 * this.refreshDelay.getValue()) {
            ...
            this.lavaPosition.clear();
            Hud.onMessage(Text.literal(Formatting.BOLD + "WormLavaESP"), Text.literal("正在刷新岩浆点位"));
               ...
               () -> {
                  if (MinecraftClient.getInstance().player != null) {
                     for (BlockPos pos : BlockPos.iterate(
                        ...
                        new BlockPos(MinecraftClient.getInstance().player.getBlockX() - 128, 64, MinecraftClient.getInstance().player.getBlockZ() - 128)
                     )) {
                        if (MinecraftClient.getInstance().world.getBlockState(pos).getBlock() == Blocks.LAVA
                           && pos.getX() >= 513
                           && pos.getZ() >= 513) {
                     ...
                     Hud.onMessage(Text.literal(Formatting.BOLD + "WormLavaESP"), Text.literal("发现" + this.lavaPosition.size() + "个点位"));
                  }
               }
            );
            thread.start();
